...
const AnhChatOne = (props) => {
    ...
    const link = () => {
        history.push('/LinkChatOne', {chatID : chatID})
    }

    const getUsers = async () => {
        const querySanp = await firebase.firestore().collection("chatrooms").doc(chatID).collection("messages")
            .orderBy("downloadURL", "asc").get()
