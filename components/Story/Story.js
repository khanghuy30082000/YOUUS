...
const Story = (story, isWith, storyId) => {
...
{isWith ? ( <View style={{    display: 'flex',
  ...
  marginLeft: 8,top:-5,marginTop:10}}>
          ...
          <Text style={styles.date}>
            {/* {moment(story.created.toDate()).format('DD/MM hh:ss')} */}
          </Text>
     ...
     {story.story ? (<Text style={styles.post}>{story.story}</Text>):(<Text style={{marginBottom:0}}></Text>)} 
      <View >
        {/* {story.postPicture && (
          <TouchableOpacity onPress={() => openPost(storyId)} >
            <Image 
              uri={story.postPicture}
    ...
    <View style={styles.reactionContainer}>
      <View style={{
        ...
        {/* <TouchableOpacity onPress={() => addLikesToDb(userId)} style={{ marginStart: 20, }}>
          {(postPicture.usersWhoLiked || []).find((user) => user === userId) ===
            undefined ? (
...
<Text style={styles. reactionTextPi}>?</Text>
            ...
            <View style={styles.reactionSection}>
              ...
              <Text style={styles. reactionTextPi}>{story.usersWhoLiked.length} </Text>
            </View>
...
isWithUserName: { marginLeft: 5, fontSize: 14, fontWeight: 'bold' },
